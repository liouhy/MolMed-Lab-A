...
# create tables with ranking powers for the circadian clock 
# create tables with ridge results for the circadian clock
for j in condition_dict:
    ...
    for i in df:
        ...
        #env = wAn.get_envelope(detr_signal, window_size = 36)
        
        df_preprocess[i] = norm_signal
    ...
    for i in df_preprocess:
        norm_signal = df_preprocess[i].dropna()
        wAn.compute_spectrum(norm_signal, do_plot = False) #False to not show each spectrum plot
    ...
    # reset indices to save them in a column
    powers_series = powers_series.reset_index()
    powers_series.to_csv('wider_table/powerseries/%s_powerseries.csv'% file_name,index=False)
